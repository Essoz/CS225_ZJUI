...
template<class T> void AList<T>::pushback (T value)
{
    ...
    append (value);     // Directly add the new value to the end
}

template<class T> void AList<T>::pushfront (T value)
{
    // Also, we can not further increase the efficiency
    insert (0, value);  // Directly add the new value at the front of array
...
{
    int length;
    length = getlength();           // Get the length of array
