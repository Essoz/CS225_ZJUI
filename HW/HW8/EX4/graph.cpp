...
template<class T> vertex<T>::vertex(T item, vertex<T> *pt_next, vertex<T> *pt_prev)
{
    ...
    next = pt_next;
    previous = pt_prev;
    edgelist = 0;   // it is initialized to null
}
...
/*return a class edgelist contain all the edge from this vertex*/
...
template<class T> int vertexlist<T>::countedges(void)
{
    ...
    for (int i = 0; i < numvertices; i++)   //for every vertex in the list 
    {
...
/* To build list of outgoing edges from a given vertex, a new edge sequence is created for the output, then by scanning the associated edgelist edges (pairs of vertices) are added one by one. 
*in other words, this function will return all the edge that start from the vertex of this item
*/
template<class T> edgelist<T> * vertexlist<T>::outgoingedges(T item)
{
    edgelist<T> * result = new edgelist<T>;
    vertex<T> * ptv = (*dummy).getnext();   //the first vertex
    for (int i = 0; i < numvertices; i++)
    {
        if ((*ptv).getitem() == item)   //first find the vertex with the item
        {
            elist<T> * edges = (*ptv).getedges(); //get all the neighbor edge from this vertex, I mean the edge that starts from this vertex
            if (edges != 0)
                (*edges).addall(item, result); //actully just copy all the information from elist to edgelist
            return result;
        }
        ptv = (*ptv).getnext();
    }
...
/* only for testing
*print every edges from one vertex, loop through all vertex
 */
template<class T> void vertexlist<T>::prettyprint(void)
{
    vertex<T> *ptv = (*dummy).getnext();    //find the first vertex
    for (int i = 0; i < numvertices; i++)
    {
...
/*This is the only function used for adding vertex in the elist*/
...
*in other words, add all neighbor edge from one item to the main edge list
*/
...
/*add doublely since it is undirectely*/
...
/*delete doublely since it is undirectely*/
