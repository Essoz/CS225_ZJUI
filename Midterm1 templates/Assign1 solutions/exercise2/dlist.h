...
public:
    ...
    // f has return type U, and a single parameter of type T. note T is from DList<T>.
    // g has return type U, and two parameters of type U.
    template<class U> U src(U e ,U f(T value), U g(U, U));
};
...
template <class T> template<class U> U DList<T>::src(U e ,U f(T), U g(U, U))
{
    if (numitems == 0)
        return e;
    if (numitems == 1)
    ...
    DList<T>* list1 = new DList<T>();
    DList<T>* list2 = new DList<T>();
    for (int i = 0; i < mid; ++i){
        list1->append(getitem(i + 1));
    }
    for (int i = mid; i < numitems; ++i){
        list2->append(getitem(i + 1));
    }
