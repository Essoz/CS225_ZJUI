...
template<class T> void DList<T>::_reverse_check_(void){
    // assert(1 == 0);
    cout << "(1) Reverse once" << endl;
    this->reverse();
    this->_pprint_();


    assert(this->first()->getprevious() == dummy);
    assert(this->last()->getnext() == dummy);
    cout << "(2) DList Property Check Pass" << endl;

    cout << "(3) Reverse again" << endl;
    this->reverse();
    this->_pprint_();
